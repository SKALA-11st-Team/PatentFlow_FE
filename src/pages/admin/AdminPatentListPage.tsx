...
export function AdminPatentListPage() {
  ...
  async function handleSavePatent(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setPendingPdfInputKey((key) => key + 1);
  ...
  // PATENT-08(항목3): 브라우저 기본 file input 대신 버튼형 label을 쓰므로, 등록/취소 시
  // 같은 파일 재선택이 가능하도록 input을 key로 리마운트한다.
  const [pendingPdfInputKey, setPendingPdfInputKey] = useState(0);
    ...
    >
        ...
        <form className="patent-edit-form" onSubmit={handleSavePatent}>
          ...
          <div className="context-suggestion-row">
            <div>
              ...
              {pendingPdfFile ? (
                <span className="pdf-file-chip">
                  <span aria-hidden="true" className="pdf-file-chip-icon">PDF</span>
                  {pendingPdfFile.name}
                  <button
            ...
            <label className="btn btn-secondary">
              {pendingPdfFile ? "다른 파일 선택" : "PDF 파일 선택"}
              <input
                accept=".pdf,application/pdf"
                aria-label="등록할 특허 PDF 파일 선택"
                hidden
                key={pendingPdfInputKey}
      ...
      >
        <div className="inline-action-group csv-action-row">
          ...
          {/* 항목6: 가져오기도 내보내기와 동일한 버튼 스타일(label이지만 .btn 클래스 공유). */}
          <label className="btn btn-secondary">
            CSV 가져오기
